<script lang="ts" setup>
    ...
    const handleValidate=()=>{
        if(formRef.value) {
            validate(formRef.value, createPromoSchema,
                async(body: any)=>{
                    const promoQuery = route.query.promo_id;

                    body.name = body.code;
                    body.is_active = isActive.value;
                    if(!!promoQuery) {
                        const res = await sendData(body, `/promotions/promocode/${promoQuery}`, 'PATCH');
                        await router.push(`/admin/hotel/promocodes/`);
                    } else {
                        const res = await sendData(body, '/promotions/promocode');
                        await router.push(`/admin/hotel/promocodes/?created_promo=${res.id}`);
                    }
    ...
    onMounted(async()=>{
        ...
        const promoQuery = route.query.promo_id;
        if(!!promoQuery) {
            const res = await getData(`/promotions/promocode/${promoQuery}`);
            randomizedPromo.value = res.name;
            descriptionValue.value = res.description;
        }
    });
</script>
...
<template>
    <section>
        <div class="promocode-create-section">
            <div class="promocode-create-section__container container_admin">
                <form class="promocode-create-section__form" ref="formRef">
                    <div class="promocode-create-section__inputs">
                        <ElementInput 
                    ...
                    <div class="promocode-create-section__btns">
                        <ElementCheckbox
